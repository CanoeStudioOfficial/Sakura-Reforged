...
public class GuiDistillation extends GuiContainer {
    ...
    private TileEntityDistillation tileDistillation;
    private int guiLeft;
    private int guiTop;
    ...
    private void updateButtonStates() {
        boolean hasInputFluid = tileDistillation.getTank() != null && tileDistillation.getTank().getFluidAmount() > 0;
        boolean hasOutputFluid = tileDistillation.getResultTank() != null && tileDistillation.getResultTank().getFluidAmount() > 0;
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == BUTTON_CLEAR_INPUT) {
            CommonProxy.getNetwork().sendToServer(new PacketClearFluid(tileDistillation.getPos(), 0));
        } else if (button.id == BUTTON_CLEAR_OUTPUT) {
            CommonProxy.getNetwork().sendToServer(new PacketClearFluid(tileDistillation.getPos(), 1));
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTickTime, int x, int y) {
        ...
        if (this.tileDistillation.getTank().getFluid() != null) {
            FluidTank fluidTank = this.tileDistillation.getTank();
        ...
        if (this.tileDistillation.getResultTank().getFluid() != null) {
            FluidTank fluidTank = this.tileDistillation.getResultTank();
            int heightInd = (int) (68 * ((float) fluidTank.getFluidAmount() / (float) fluidTank.getCapacity()));
    ...
    private int getCookProgressScaled(int pixels) {
        int i = this.tileDistillation.getField(0);
    ...
    private void drawFluidTooltips(int mouseX, int mouseY) {
        ...
        if (GuiFluidHelper.isMouseOver(mouseX, mouseY, k + 18, l + 10, 16, 68)) {
            List<String> tooltip = GuiFluidHelper.getFluidTooltip(this.tileDistillation.getTank());
            this.drawHoveringText(tooltip, mouseX, mouseY);
        }

        if (GuiFluidHelper.isMouseOver(mouseX, mouseY, k + 89, l + 10, 16, 68)) {
            List<String> tooltip = GuiFluidHelper.getFluidTooltip(this.tileDistillation.getResultTank());
            this.drawHoveringText(tooltip, mouseX, mouseY);
        }
    }
}
